...
import sub.Course;
import sub.CourseStatistics;
import sub.Student;
import sub.CourseStatsComparators; // user defined packages
...
public class LearningPlatform {
    ...
    public void findStudentCourseDetails(int inputId) {
        ...
        if (student == null) {
            ...
            System.out.printf("%s points : Java = %s; DSA = %s; Database = %s; Spring = %s \n", inputId, courses.get(0).getPoint(),
                courses.get(1).getPoint(), courses.get(2).getPoint(), courses.get(3).getPoint());
        }
    }
    ...
    public void showCourseLearners(String courseName) {
        ...
        if (courseStatistics.getPopularity() != 0) {
            ...
            ArrayList<Integer> sortedStudentId = enrolledStudents.stream()
                .sorted(Comparator.comparingDouble(id -> students.get(id).getStudentProgress(courseStatistics
